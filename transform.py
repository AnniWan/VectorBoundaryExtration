...
class Compose(object):
    ...
    def __call__(self, image, label,gray):
        for t in self.segtransform:
            image, label,gray = t(image, label,gray)
        return image, label,gray
...
class ToTensor(object):
    ...
    def __call__(self, image, label,gray):
        ...
        gray = [torch.from_numpy(gray[0]),torch.from_numpy(gray[1])]
...
class Resize(object):
    ...
    def __call__(self, image, label,gray):
        ...
        gray[0] = cv2.resize(gray[0], self.size[::-1], interpolation=cv2.INTER_LINEAR)
        gray[1] = cv2.resize(gray[1], self.size[::-1], interpolation=cv2.INTER_LINEAR)
        return image, label,gray
...
class Crop(object):
    ...
    def __call__(self, image, label,gray):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            gray[0] =cv2.copyMakeBorder(gray[0], pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, 0)
            gray[1] = cv2.copyMakeBorder(gray[1], pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, 0)
        h, w = label.shape
        ...
        gray[0] =gray[0][h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        gray[1] = gray[1][h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        return image, label,gray
