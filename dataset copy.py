...
class PolyData(Dataset):
    def __init__(self, split='train', point_length=20,data_root=None, data_list=None, transform=None):
        self.split = split
    ...
    def __getitem__(self, index):
       ...
       # angle_field= self.init_angle_field(points,[image.shape[0],image.shape[1]], line_width=1)
        ...
        if self.split=='test':
            return image, label, filed, points,name
        else:
            return image, label, filed, points
    ...
    def point_resize(self,points,o_image,image):
        ...
        oo=cv2.resize(o_image,[300,300])
        cv2.drawContours(oo,[points],-1,(0,255,0),3)
